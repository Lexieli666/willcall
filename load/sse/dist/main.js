...
import { SseConnection } from "./sseClient.js";
/**
...
async function createEvent() {
    ...
    if (!response.ok)
        throw new Error(`could not create an event: ${response.status}`);
    return (await response.json());
}
async function seatIds(eventId) {
...
async function main() {
    ...
    if (seats.length === 0)
        throw new Error('the event has no available seats');
    ...
    for (let i = 0; i < CONNECTIONS; i++) {
        const connection = new SseConnection(`${BASE_URL}/api/events/${eventId}/stream?userRef=load-${i.toString().padStart(8, '0')}`, counters, propagation, recordError);
        connections.push(connection);
        running.push(connection.start());
        if (gapBetweenOpens >= 1)
            await delay(gapBetweenOpens);
        else if (i % 50 === 0)
            await delay(1);
    }
    ...
    while (Date.now() < deadline && changeIndex < seats.length) {
        ...
        for (let i = 0; i < CHANGES_PER_SEC && changeIndex < seats.length; i++) {
            const seatId = seats[changeIndex++];
            batch.push(fetch(`${BASE_URL}/api/events/${eventId}/holds`, {
                method: 'POST',
                headers: {
                ...
                .catch((cause) => recordError(`driver: ${String(cause)}`)));
        }
    ...
    const withMemory = {
        ...
        retainedHeap: {
            ...
            method: 'Heap retained after an explicit collection, with the connections open, minus the same ' +
    ...
    if (retainedPerConnection !== null) {
        console.log(`retained heap after close: ${(heapAfterClose / 1024 / 1024).toFixed(1)} MiB` +
