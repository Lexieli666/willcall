...
| FIFO inversion rate in the waiting room | 0.000000% — 5 out-of-order pairs among — possible, over 18,495 admitted buyers | < 1% expected; publish whatever it is | met | `load/results/2026-09-20/fairness-215323/fairness.json` |
| Game day: faults injected while traffic flowed | 4 scenarios (exhaust-pool, inject-latency, kill-replica, restart-redis); invariants held after 4/4, 0 server errors; 2 re-run to verify a fix | every scenario survives with invariants intact and a written postmortem | met | `docs/incidents/` |
